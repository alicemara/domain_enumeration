...
for i in $(awk '{print $2}' "${target_domain}_subdomainips.txt"); do shodan host $i; sleep 1; done | tee ${target_domain}_shodanresults.txt

echo "Results from Dig:"
for i in $(cat ${target_domain}_subdomainlist.txt); do dig any $i; done | tee ${target_domain}_digresults.txt
